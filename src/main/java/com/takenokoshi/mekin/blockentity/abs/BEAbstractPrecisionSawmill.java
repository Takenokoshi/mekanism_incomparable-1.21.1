...
import com.takenokoshi.mekin.recipe.cached.AdvancedSawingCachedRecipe;
...
import static mekanism.common.tile.machine.TileEntityPrecisionSawmill.NOT_ENOUGH_SPACE_SECONDARY_OUTPUT_ERROR;

public abstract class BEAbstractPrecisionSawmill extends TileEntityProgressMachine<SawmillRecipe>
        implements ItemRecipeLookupHandler<SawmillRecipe>, IHasMachineEnergyContainer,
    ...
    LimitChangedOutputInventorySlot mainOutputSlot;
    LimitChangedOutputInventorySlot subOutputSlot;
    EnergyInventorySlot energySlot;
    private long clientEnergyUsed = 0L;
    private int stackUpgrade = 0;

    protected final AdvancedItemInputHandler inputHandler;
    protected final IOutputHandler<ItemStack> mainOutputHandler;
    protected final IOutputHandler<MekInChanceOutput> subOutputHandler;

    protected BEAbstractPrecisionSawmill(Holder<Block> blockProvider, BlockPos pos, BlockState state,
            int baselineMaxOperations) {
        super(blockProvider, pos, state, TRACKED_ERROR_TYPES, 200);

        configComponent.setupItemIOConfig(Collections.singletonList(inputSlot), List.of(mainOutputSlot, subOutputSlot),
                energySlot, false);
        ...
        mainOutputHandler = new ItemOutputHandler(mainOutputSlot, RecipeError.NOT_ENOUGH_OUTPUT_SPACE);
        subOutputHandler = initSubOutputHandler(subOutputSlot, NOT_ENOUGH_SPACE_SECONDARY_OUTPUT_ERROR);
        inputSlot.setSupplyingStackSetter(inputHandler::setSuppliedStack);
    }

    protected abstract IOutputHandler<MekInChanceOutput> initSubOutputHandler(IInventorySlot slot,
    ...
    protected IInventorySlotHolder getInitialInventory(IContentsListener listener,
        ...
        builder.addSlot(mainOutputSlot = LimitChangedOutputInventorySlot.at(recipeCacheUnpauseListener, 116, 35,
        ...
        builder.addSlot(subOutputSlot = LimitChangedOutputInventorySlot.at(recipeCacheUnpauseListener, 132, 35,
                initItemSlotCapacity()))
                .tracksWarnings(slot -> slot.warning(WarningType.NO_SPACE_IN_OUTPUT,
                        getWarningCheck(RecipeError.NOT_ENOUGH_OUTPUT_SPACE)));
        builder.addSlot(
    ...
    @Override
    public @Nullable SawmillRecipe getRecipe(int arg0) {
        return findFirstRecipe(inputHandler);
    }

    @Override
    public @NotNull CachedRecipe<SawmillRecipe> createNewCachedRecipe(@NotNull SawmillRecipe recipe, int arg1) {
        return new AdvancedSawingCachedRecipe(recipe, recheckAllRecipeErrors, inputHandler, mainOutputHandler,
                subOutputHandler)
    ...
    @Override
    public @Nullable IRecipeViewerRecipeType<SawmillRecipe> recipeViewerType() {

...
import com.takenokoshi.mekut.recipe.inputcache.MUAbstractInputRecipeCache;
...
public abstract class MekInTripleInputRecipeCache<INPUT_A, INGREDIENT_A extends InputIngredient<INPUT_A>, INPUT_B, INGREDIENT_B extends InputIngredient<INPUT_B>, INPUT_C, INGREDIENT_C extends InputIngredient<INPUT_C>, RECIPE extends MekanismRecipe<?> & TriPredicate<INPUT_A, INPUT_B, INPUT_C>, CACHE_A extends IInputCache<INPUT_A, INGREDIENT_A, RECIPE>, CACHE_B extends IInputCache<INPUT_B, INGREDIENT_B, RECIPE>, CACHE_C extends IInputCache<INPUT_C, INGREDIENT_C, RECIPE>>
    ...
    protected MekInTripleInputRecipeCache(MekALRecipeType<?, RECIPE, ?> recipeType,
    ...
    /**
     * Checks is there is a matching recipe with the given inputs. This method
     * exists as a helper for insertion predicates and will return true if inputA is
     * not empty and
     * inputB and inputC is empty without doing any extra validation on inputA. If
     * however inputA is not empty and only one of the other two inputs is empty
     * this will do
     ...
     *
     * @return {@code true} if there is a match or if inputA is not empty and inputB
     *         and inputC are both empty.
     *
     * @apiNote If you are trying to insert inputA call this method, otherwise call
     *          {@link #containsInputBAC(Level, Object, Object, Object)} or
     *          {@link #containsInputCAB(Level, Object, Object, Object)} depending
     *          on which input is trying to be inserted.
     */
    public boolean containsInputABC(@Nullable Level world, INPUT_A inputA, INPUT_B inputB, INPUT_C inputC) {
        return containsGrouping(world, inputA, inputAExtractor, cacheA, complexIngredientA, inputB, inputBExtractor,
    ...
    /**
     * Checks is there is a matching recipe with the given inputs. This method
     * exists as a helper for insertion predicates and will return true if inputB is
     * not empty and
     * inputA and inputC is empty without doing any extra validation on inputB. If
     * however inputB is not empty and only one of the other two inputs is empty
     * this will do
     ...
     * @param inputC Recipe input C.
     *
     * @return {@code true} if there is a match or if inputB is not empty and inputA
     *         and inputC are both empty.
     *
     * @apiNote If you are trying to insert inputB call this method, otherwise call
     *          {@link #containsInputABC(Level, Object, Object, Object)} or
     *          {@link #containsInputCAB(Level, Object, Object, Object)} depending
     *          on which input is trying to be inserted.
     */
    public boolean containsInputBAC(@Nullable Level world, INPUT_A inputA, INPUT_B inputB, INPUT_C inputC) {
        return containsGrouping(world, inputB, inputBExtractor, cacheB, complexIngredientB, inputA, inputAExtractor,
    ...
    /**
     * Checks is there is a matching recipe with the given inputs. This method
     * exists as a helper for insertion predicates and will return true if inputC is
     * not empty and
     * inputA and inputB is empty without doing any extra validation on inputC. If
     * however inputC is not empty and only one of the other two inputs is empty
     * this will do
     ...
     * @param inputC Recipe input C.
     *
     * @return {@code true} if there is a match or if inputC is not empty and inputA
     *         and inputB are both empty.
     *
     * @apiNote If you are trying to insert inputC call this method, otherwise call
     *          {@link #containsInputABC(Level, Object, Object, Object)} or
     *          {@link #containsInputBAC(Level, Object, Object, Object)} depending
     *          on which input is trying to be inserted.
     */
    public boolean containsInputCAB(@Nullable Level world, INPUT_A inputA, INPUT_B inputB, INPUT_C inputC) {
        return containsGrouping(world, inputC, inputCExtractor, cacheC, complexIngredientC, inputA, inputAExtractor,
                cacheA, complexIngredientA,
                inputB, inputBExtractor, cacheB, complexIngredientB);
    }

    /**
     * Helper to check if a cache contains a given input grouping, or if not, if the
     * complex recipe fallback set contains a matching recipe. This method is mainly
     * used
     * for purposes of implementing insertion predicates, so it has the following
     * behaviors. This allows it to short circuit in cases where we already know the
     * input is
     * valid (the last case in the below list).
     * <ul>
     * <li>If only the first input is empty: This will check if input two and three
     * are contained.</li>
     * <li>If only the second input is empty: This will check if input one and three
     * are contained.</li>
     * <li>If only the third input is empty: This will check if input one and two
     * are contained.</li>
     * <li>If only the first and third input is empty: This will check if the second
     * input is contained.</li>
     ...
     *         matches.
     *
     * @implNote Lookups up the recipe first from the A input map (the fact that it
     *           is A is arbitrary and just as well could be B or C).
     */
    @Nullable
    public RECIPE findFirstRecipe(@Nullable Level world, INPUT_A inputA, INPUT_B inputB, INPUT_C inputC) {
        ...
        initCacheIfNeeded(world);
        // Lookup a recipe from the A input map (the fact that it is A is arbitrary, it
        // just as well could be B or C)
        RECIPE recipe = findFirstRecipe(inputA, inputB, inputC, cacheA.getRecipes(inputA));
        // if there is no recipe, then check if any of our complex recipes (either a, b,
        // or c being complex) match
    ...
    public static class MekInItemFluidChemical<RECIPE extends MekanismRecipe<?> & TriPredicate<ItemStack, FluidStack, ChemicalStack>>
            extends
            MekInTripleInputRecipeCache<ItemStack, ItemStackIngredient, FluidStack, FluidStackIngredient, ChemicalStack, ChemicalStackIngredient, RECIPE, ItemInputCache<RECIPE>, FluidInputCache<RECIPE>, ChemicalInputCache<RECIPE>> {

        public MekInItemFluidChemical(MekALRecipeType<?, RECIPE, ?> recipeType,
        ...
        public static MekInItemFluidChemical<ItemStackFluidChemicalToItemStackRecipe> toItem(
                MekALRecipeType<?, ItemStackFluidChemicalToItemStackRecipe, ?> recipeType) {
            return new MekInItemFluidChemical<>(recipeType,
